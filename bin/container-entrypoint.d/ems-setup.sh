...
source /opt/bin/urlencode.sh
url_encode $DB_PASSWORD DB_PASSWORD_ENCODED
set +o allexport

if [ \${1:-list} = sql ] || [ \${1:-list} = dump ] ; then
  if [ \${DB_DRIVER:-mysql} = mysql ] ; then
    if [ \${1:-list} = sql ] ; then
      mysql --port=\$DB_PORT --host=\$DB_HOST --user=\$DB_USER --password=\$DB_PASSWORD_ENCODED \$DB_NAME
    else
      mysqldump --port=\$DB_PORT --host=\$DB_HOST --user=\$DB_USER --password=\$DB_PASSWORD_ENCODED \$DB_NAME
    fi;
